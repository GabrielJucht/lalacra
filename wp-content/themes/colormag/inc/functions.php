...
function colormag_footer_copyright() {
   ...
   $colormag_footer_copyright = '<div class="copyright">Copyright © 2016 La Lacra. Todos los derechos reservados.</div>';
...
if ( !function_exists('colormag_colored_category') ) :
   function colormag_colored_category() {
      ...
      if($categories) {
         ...
         $output .='</span>';
		 $output .= '<a class="autor" href="'.get_author_posts_url( get_the_author_meta( 'ID' ) ).'">'.get_the_author().'</a>' ;
	     $time_string = '<time class="entry-date published" datetime="%1$s">%2$s</time>';
			$time_string = sprintf( $time_string,
				esc_attr( get_the_date( 'c' ) ),
				esc_html( get_the_date() ),
